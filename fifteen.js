document.addEventListener("DOMContentLoaded", function () {
  ...
  for (var tileNumber = 0; tileNumber < 15; tileNumber++) {
    ...
    singleTile.style.backgroundPosition =
      "-" + horizontalPosition + "px -" + verticalPosition + "px";
    singleTile.style.backgroundImage =
  ...
  function canTileBeMoved(tileX, tileY) {
    var adjacentToEmptyX =
      tileX === emptySpaceLocation.x &&
      Math.abs(tileY - emptySpaceLocation.y) === 1;
    var adjacentToEmptyY =
      tileY === emptySpaceLocation.y &&
      Math.abs(tileX - emptySpaceLocation.x) === 1;
  ...
  function isPuzzleSolved() {
    for (var i = 0; i < allPuzzleTiles.length; i++) {
      ...
      if (
        parseInt(currentTile.style.left) !== correctXPosition ||
